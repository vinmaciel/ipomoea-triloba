...
import br.usp.pcs.securetcg.library.config.SecurityConstantsFalse2;
 ...
 *
 */
public final class SystemParameterFalse2 {

	/* Singleton */
	private SystemParameterFalse2() {
		 this.n = new BigInteger(SecurityConstantsFalse2.n).toByteArray();
		 this.p = new BigInteger(SecurityConstantsFalse2.q).toByteArray();
		 this.g = new byte[7][];
		 for(int i = 0; i < g.length; i++)
			 this.g[i] = new BigInteger(SecurityConstantsFalse2.r[i]).toByteArray();
		 this.h = new BigInteger(SecurityConstantsFalse2.s).toByteArray();
		 this.z = new BigInteger(SecurityConstantsFalse2.z).toByteArray();
	}
	private static SystemParameterFalse2 instance = null;
	
	/* Lazy singleton instantiation */
	public static SystemParameterFalse2 get() {
		if(instance == null) instance = new SystemParameterFalse2();
		return instance;
	}
